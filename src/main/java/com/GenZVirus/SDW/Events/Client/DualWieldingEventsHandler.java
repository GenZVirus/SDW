...
public class DualWieldingEventsHandler {
	...
	public static void RenderHands(RenderHandEvent event) {
		...
		if (event.getHand() == Hand.MAIN_HAND)
			renderItemInFirstPerson(mc.player, event.getPartialTicks(), event.getInterpolatedPitch(), Hand.MAIN_HAND, event.getSwingProgress(), itemStackMainHand, event.getEquipProgress(), event.getMatrixStack(), event.getBuffers(), event.getLight());
		if (event.getHand() == Hand.OFF_HAND)
			renderItemInFirstPerson(mc.player, event.getPartialTicks(), event.getInterpolatedPitch(), Hand.OFF_HAND, event.getSwingProgress(), itemStackOffHand, event.getEquipProgress(), event.getMatrixStack(), event.getBuffers(), event.getLight());
	}
